...
def запустити_товар_дня(токен: str, chat_id: str):
    ...
    заголовок = (
        f"🛒 <b>Товар дня ATB Market — {сьогодні_str}</b>\n"
        f"Знайдено товарів зі знижкою: {len(нові)}\n"
        "─────────────────────"
    ) if якщо_перший else (
        f"🔔 <b>ATB Market — нові товари дня ({час_str})</b>\n"
...
def запустити_моніторинг_акцій(токен: str, chat_id: str):
    ...
    if перший_запуск:
        log.info("Перший запуск — зберігаємо %d акцій як відомі.", len(акції))
        for а in акції:
            збережені[а["slug"]] = {
        ...
        стан["seen_slugs"] = [а["slug"] for а in акції]
        ...
        tg_text(токен, chat_id,
            f"✅ <b>ATB Promo Monitor запущено!</b>\n"
            f"Зараз на сайті активних акцій: <b>{len(акції)}</b>\n"
            f"Буду сповіщати про нові каталоги знижок 🔔\n"
            "─────────────────────\n"
            + рядки_акцій
        )
        return

    нові = [а for а in акції if а["slug"] not in відомі]
